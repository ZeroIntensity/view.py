...
<Admonition level="question" title="What is a WebSocket?">
...
You can add `query` inputs and path parameters to a `websocket` route, but not a `body` input. This isn't a limitation with view.py, but it's not supported by the protocol itself.
</Admonition>
...
<Admonition level="info" title="Client Disconnect">
