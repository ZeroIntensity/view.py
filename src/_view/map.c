...
void *map_get(map *m, const char *key) {
  ...
  while (m->items[index] != NULL) {
    if (!strcmp(key, m->items[index]->key))
...
map *map_new(size_t inital_capacity, map_free_func dealloc) {
  ...
  m->items = calloc(inital_capacity, sizeof(pair));
  if (!m->items)
    return (map *)PyErr_NoMemory();
  m->dealloc = dealloc;
  return m;
...
static int expand(map *m) {
  ...
  pair **items = calloc(new_capacity, sizeof(pair));
  if (!items) {
    PyErr_NoMemory();
    return -1;
  }
  ...
  for (int i = 0; i < m->capacity; i++) {
    pair *item = m->items[i];
    if (item) {
      if (set_entry(items, new_capacity, item->key, item->value, NULL,
                    m->dealloc) < 0) {
        return -1;
      };
      free(item);
...
void map_set(map *m, const char *key, void *value) {
  if (m->len >= m->capacity / 2)
    expand(m);
  set_entry(m->items, m->capacity, key, value, &m->len, m->dealloc);
}
